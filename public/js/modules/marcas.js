...
const MarcasModule = {
  ...
  _render(lista) {
    ...
    tbody.innerHTML = lista.map((marca, i) => {
      ...
      return `
        <tr>
          ...
          <td>
            <button class="btn-action btn-action-edit" onclick="MarcasModule.openEdit(${marca.id_marca})" title="Editar"><i class="bi bi-pencil-fill"></i></button>
            <button class="btn-action btn-action-delete" onclick="MarcasModule.confirmDel(${marca.id_marca}, '${escapeHtml(marca.nombre)}')" title="Eliminar"><i class="bi bi-trash3-fill"></i></button>
          </td>
        </tr>`;
    }).join('');
  },
  ...
  _filter() {
    ...
    this._render(AppState.marcas.filter(marca =>
  ...
  _openModal(mode, marca = null) {
    ...
    document.getElementById('mDescripcion').value = isEdit ? (marca.descripcion || '') : '';
    clearErrors(['mNombre']);
    openOverlay('modalMarcaOverlay');
  },

  openEdit(id) {
    const marca = AppState.marcas.find(m => Number(m.id_marca) === Number(id));
  ...
  async _save() {
    ...
    try {
      await http(isEdit ? `/api/marcas/${id}` : '/api/marcas', isEdit ? 'PUT' : 'POST', payload);
